...
proc rawSend(q: PRawChannel, data: pointer, typ_size: int) =
  ## Adds an `item` to the end of the queue `q`.
  var cap = q.mask+1
  if q.count >= cap:
    # start with capacity for 2 entries in the queue:
    if cap == 0: cap = 1
    var n = cast[PBytes](allocShared0(cap*2*typ_size))
    var z = 0
    var i = q.rd
    var c = q.count
    while c > 0:
      dec c
      copyMem(addr(n[z*typ_size]), addr(q.data[i*typ_size]), typ_size)
      i = (i + 1) and q.mask
      inc z
    if q.data != nil:
    ...
    q.wr = q.count
    q.rd = 0
  copyMem(addr(q.data[q.wr * typ_size]), data, typ_size)
  inc q.count
  q.wr = (q.wr + 1) and q.mask

proc rawRecv(q: PRawChannel, data: pointer, typ_size: int) =
  doAssert q.count > 0, "rawRecv"
  dec q.count
  copyMem(data, addr(q.data[q.rd * typ_size]), typ_size)
...
proc sendImpl(q: PRawChannel, typ_size: int, msg: pointer, noBlock: bool): bool =
  assert q.mask != ChannelDeadMask: "cannot send message; thread died"
  acquire(q.lock)
  if q.maxItems > 0:
  ...
  rawSend(q, msg, typ_size)
  q.elemTypeSize = typ_size
  release(q.lock)
  signal(q.cond)
  result = true
...
proc send*[TMsg](c: var Channel[TMsg], msg: sink TMsg) {.inline.} =
  ...
  discard sendImpl(cast[PRawChannel](addr c), sizeof(msg), unsafeAddr(msg), false)
...
proc trySend*[TMsg](c: var Channel[TMsg], msg: sink TMsg): bool {.inline.} =
  ...
  result = sendImpl(cast[PRawChannel](addr c), sizeof(msg), unsafeAddr(msg), true)
  when defined(gcDestructors):
    if result:
      wasMoved(msg)

proc llRecv(q: PRawChannel, res: pointer, typ_size: int) =
  q.ready = true
  while q.count <= 0:
    wait(q.cond, q.lock)
  q.ready = false
  if typ_size != q.elemTypeSize:
    release(q.lock)
    raise newException(ValueError, "cannot receive message of wrong size (or type)")
  rawRecv(q, res, typ_size)
...
proc recv*[TMsg](c: var Channel[TMsg]): TMsg =
  ...
  acquire(q.lock)
  llRecv(q, addr(result), sizeof(result))
  release(q.lock)
...
proc tryRecv*[TMsg](c: var Channel[TMsg]): tuple[dataAvailable: bool,
  ...
  if q.mask != ChannelDeadMask:
    if tryAcquire(q.lock):
      if q.count > 0:
        llRecv(q, addr(result.msg), sizeof(result.msg))
