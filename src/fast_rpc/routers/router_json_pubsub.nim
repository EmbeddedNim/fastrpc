...
type
  ...
  RpcProc* = proc(input: JsonNode, sender: SocketClientSender): JsonNode {.gcsafe.}
...
proc createRpcRouter*(): RpcRouter =
...
proc route*(router: RpcRouter, node: JsonNode, extraData: SocketClientSender): JsonNode {.gcsafe.} =
  ...
  if rpcProc.isNil:
    ...
    try:
      ...
      let res = rpcProc(jParams, extraData)
